...
chunks = pd.read_csv(midifiles, compression='gzip', iterator=True, chunksize=1152)
...
for df in chunks:
    ...
    input_seq = autograd.Variable(dtype(np.array(input_seq)), requires_grad = False).permute(2,0,1) 
    ...
    pred = model(input_seq, future = 500)
    
    # Write to CSV
    with open(file_name + '.csv',"a") as f:
        pred = pred.type(torch.IntTensor)[-1,:,:].squeeze(0).data.tolist()
